...
public class PregenLogic implements ICommandLogic {
    ...
    public void handleCommand (MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if (args[1].equals("help")) { //mde pregen help
            sender.addChatMessage(new TextComponentString(TextFormatting.AQUA+"/mde pregen start <player> <x-radius> <z-radius>"+TextFormatting.GOLD+": Generates a "+TextFormatting.LIGHT_PURPLE+"x-radius"+TextFormatting.GOLD+" by "+TextFormatting.LIGHT_PURPLE+"z-radius"+TextFormatting.GOLD+" chunk square centered on "+TextFormatting.LIGHT_PURPLE+"player"+TextFormatting.GOLD+"."));
            sender.addChatMessage(new TextComponentString(TextFormatting.AQUA+"/mde pregen start <dimension> <x-start> <z-start> <x-end> <z-end>"+TextFormatting.GOLD+": Generates a square from the chunk coordinates ("+TextFormatting.LIGHT_PURPLE+"x-start"+TextFormatting.GOLD+","+TextFormatting.LIGHT_PURPLE+"z-start"+TextFormatting.GOLD+") to ("+TextFormatting.LIGHT_PURPLE+"x-end"+TextFormatting.GOLD+","+TextFormatting.LIGHT_PURPLE+"z-end"+TextFormatting.GOLD+") in "+TextFormatting.LIGHT_PURPLE+"dimension"+TextFormatting.GOLD+"."));
            sender.addChatMessage(new TextComponentString(TextFormatting.AQUA+"/mde pregen pause"+TextFormatting.GOLD+": Pauses all currently running pregenerations. "+TextFormatting.RED+"Paused pregens are NOT saved across server restarts!"));
            sender.addChatMessage(new TextComponentString(TextFormatting.AQUA+"/mde pregen resume"+TextFormatting.GOLD+": Resumes any paused pregenerations."));
            sender.addChatMessage(new TextComponentString(TextFormatting.AQUA+"/mde pregen cancel"+TextFormatting.GOLD+": Stops and removes all running and paused pregenerations for all dimensions."));
        } else if (args[1].equals("pause")) {
            WorldEventHandler.pregenPause = true;
            sender.addChatMessage(new TextComponentString(TextFormatting.GREEN+"Pregeneration paused!"));
            MDECore.logger.info("Pregeneration paused by command.");
        } else if (args[1].equals("resume")) {
            WorldEventHandler.pregenPause = false;
            sender.addChatMessage(new TextComponentString(TextFormatting.GREEN+"Pregeneration resumed!"));
            MDECore.logger.info("Pregeneration resumed by command.");
        } else if (args[1].equals("cancel")) {
            WorldEventHandler.pregenPause = true;
            WorldEventHandler.chunksToGenerate.clear();
            sender.addChatMessage(new TextComponentString(TextFormatting.GREEN+"All pending pregenerations canceled."));
            ...
            if (NumberUtils.isNumber(args[2])) {
                ...
                if (p == null) AbstractCommand.throwNoPlayer();
                dim = p.dimension;
            ...
            sender.addChatMessage(new TextComponentString(TextFormatting.GREEN+"Pregeneration started!"));
        } else {
